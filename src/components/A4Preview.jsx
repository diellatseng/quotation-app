...
function buildSections({ quotation, services, stages, client, contactPerson, companyInfo, total, taxAmount, grandTotal, hasChecklist, negLogs }) {
        ...
        <table style={a4.table}>
          <thead>
            <tr>
              ...
              <th style={{ ...a4.th, textAlign: 'left' }}>服務項目</th>
            </tr>
          </thead>
          <tbody />
        </table>
        ...
        <table style={{ ...a4.table, marginTop: -1 }}>
          <tbody>
            <tr style={idx % 2 === 1 ? { background: '#f9f9f7' } : {}}>
              ...
              <td style={{ ...a4.td, textAlign: 'left', fontWeight: 400, verticalAlign: 'top', paddingTop: 8 }}>
    ...
    sec.push({
      ...
      element: (
        <div style={{ ...a4.section, marginTop: 16 }} >
          <div style={a4.sectionLabel}>報價金額及款階段</div>
          <table style={a4.table}>
            <thead>
              <tr>
  ...
  sec.push({
    ...
    element: (
      <div style={a4.section}>
        <div style={a4.sectionLabel}>確認請簽名回傳</div>
        <div style={a4.sigRow}>
          <div style={a4.sigLabel1}>簽名：</div>
          <div style={a4.sigLabel2}>日期：</div>
        </div>
      </div>
    ),
  })

  // ── Appendix 1 — Service descriptions (always new page, always included) ──
  // Description column was removed from the main table; this appendix carries
  // the full detail for internal / confirmed-quotation use.
  const servicesWithDesc = services.filter(s => s.description?.trim())
  if (servicesWithDesc.length > 0) {
    sec.push({
      key: 'desc-appendix-header',
      forceNewPage: true,
      element: (
        <div style={{ marginTop: 8 }}>
          <div style={a4.header}>
            <div style={{ ...a4.docTitle, fontSize: 18 }}>服務細項說明</div>
          </div>
          <div style={a4.rule} />
          <div style={{ fontSize: 11, color: '#666', marginBottom: 16 }}>
            客戶：{client?.company_name} ／ 地號：{quotation.land_section}
          </div>
        </div>
      ),
    })
      ...
      sec.push({
        ...
        element: (
          <div style={{ marginBottom: 20 }}>
            ...
            <div
              className="a4-desc"
              style={{ ...a4.descriptionCell, padding: '8px 12px', border: '1px solid #d4d2cb', borderRadius: 2, lineHeight: 1.8 }}
  ...
  // ── Appendix 2 — Client document checklist (always new page) ──
  if (hasChecklist) {
    sec.push({
      ...
      element: (
        <div style={{ marginTop: 8 }}>
          <div style={a4.header}>
            <div style={{ ...a4.docTitle, fontSize: 18 }}>客戶準備資料清單</div>
          </div>
          <div style={a4.rule} />
          <div style={{ fontSize: 11, color: '#666', marginBottom: 16 }}>
...
const a4 = {
  ...
  sigRow: { display: 'flex', gap: 40, marginTop: 48, marginBottom: 24 },
  sigBlock: { flex: 1, border: '1px solid #d4d2cb', borderRadius: 4, padding: '16px 20px', flexDirection: 'column' },
  sigTitle: { fontSize: 12, fontWeight: 700, marginBottom: 32 },
  sigLine: { borderTop: '1px solid #333', marginBottom: 8 },
  sigLabel1: { fontSize: 12, marginTop: 6, borderBottom: '1px solid black', width: '70%' },
  sigLabel2: { fontSize: 12, marginTop: 6, borderBottom: '1px solid black', width: '30%' },
}
